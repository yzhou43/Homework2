/*This file is a lexical and syntax analyzer system for simple arithmetic expressions
...
void stmt(void) {
	expr();
	while (nextToken == LES_OP || nextToken == GRE_OP ||
	        nextToken == EQU_OP || nextToken == UNE_OP) {
		lex();
		expr();
	}
}
...
// Test if the input strings pass the lexical analysis
// Conduct the syntax analysis only if the code is lexically correct
	if (validLength == length - 1) {
		printf("No lexical error\n\n");
